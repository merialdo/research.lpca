...
h_K_matrix = np.zeros(shape=[len(models_for_dataset), len(PROPERTIES_TO_CONSIDER)+1], dtype=np.float)
...
for i in range(len(models_for_dataset)):
    ...
    rel_property_2_hitsK = dict()

    for rel_property in relation_property_2_ranks:
        ranks = relation_property_2_ranks[rel_property]
        ranks = np.array(ranks)
        rel_property_2_hitsK[rel_property] = float(np.sum(ranks <= K))/float(len(ranks))

    h_K_matrix[i, 0] = round(rel_property_2_hitsK["any"], 2) if "any" in rel_property_2_hitsK else None
    for j in range(len(PROPERTIES_TO_CONSIDER)):
        h_K_matrix[i, j+1] = round(rel_property_2_hitsK[PROPERTIES_TO_CONSIDER[j]], 2) if PROPERTIES_TO_CONSIDER[j] in rel_property_2_hitsK else None

print(h_K_matrix)
sns.heatmap(h_K_matrix,
                linewidth=0.5,
                annot=True,
                square=True,
                xticklabels=[],
