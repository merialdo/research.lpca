...
def get_h1_for(model_name, dataset_name):
    ...
    source_peers_interval_2_hits1 = dict()
    for i in range(len(keys)):
        ranks = np.array(source_peers_interval_2_ranks[keys[i]])
        hits_1_count = np.sum(ranks == 1.0)
        source_peers_interval_2_hits1[keys[i]] = float(hits_1_count)/float(len(ranks))

    return source_peers_interval_2_hits1
...
ideal_points = [source_peers_interval_2_percentage[k] for k in keys]
...
for i in range(len(all_models_names)):
    ...
    model_source_peers_interval_2_h1 = get_h1_for(model_name, dataset_name)
    y = [model_source_peers_interval_2_h1[key] for key in keys]
