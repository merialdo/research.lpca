...
def get_mrr_by_bucket(dataset_obj,
                      model_name,
                      fact_2_support,
                      support_bucket_2_facts_count):

    model_entries = read_filtered_ranks_entries_for(model_name, dataset_obj.name, entity_number=len(dataset_obj.entities))
...
for i in range(len(BUCKETS)):
    ...
    perc = round(tfidf_path_support_bucket_2_test_facts_percentage[BUCKETS[i]], 2)
    values[0][i] = perc
...
models_for_dataset = performances.get_models_supporting_dataset(dataset_name)
...
for i in range(len(models_for_dataset)):
    ...
    model_head_mrrs, model_tail_mrrs = get_mrr_by_bucket(dataset,
                                                          model,
...
all_mrrs = np.round((all_head_mrrs + all_tail_mrrs) / 2, 2)
...
plt.xlabel("Paths TF-IDF support")
plt.ylabel("Models")
plt.title("MRR by Paths TF-IDF support")
