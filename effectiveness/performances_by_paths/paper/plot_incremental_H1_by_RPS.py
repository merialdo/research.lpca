...
for fact in dataset.test_triples:
    ...
    path_support_bucket_2_count[support_bucket] += 2 # head and tail
...
def get_bucket_2_incremental_h1(dataset, model_name, fact_2_support):
    ...
    support_bucket_2_h1 = defaultdict(lambda : 0)
    ...
    for entry in model_entries:
        ...
        if head_rank_filtered == 1:
            support_bucket_2_h1[support_bucket] += 1

        if tail_rank_filtered == 1:
            support_bucket_2_h1[support_bucket] += 1

    support_bucket_2_incremental_h1 = copy.deepcopy(support_bucket_2_h1)

    for i in range(len(BUCKETS)):
        start_bucket = BUCKETS[i]
        for j in range(i+1, len(BUCKETS)):
            bucket_to_increment = BUCKETS[j]
            support_bucket_2_incremental_h1[bucket_to_increment] += support_bucket_2_h1[start_bucket]

    support_bucket_2_incremental_h1_percentage = dict()

    for bucket in BUCKETS:
        support_bucket_2_incremental_h1_percentage[bucket] = support_bucket_2_incremental_h1[bucket]/float(support_bucket_2_incremental_count[bucket])
...
for i in range(len(all_models_names)):
    ...
    bucket_2_incremental_h1_perc = get_bucket_2_incremental_h1(dataset, model_name, test_fact_2_tfidf_path_support)
    y = [bucket_2_incremental_h1_perc[key] for key in BUCKETS]
