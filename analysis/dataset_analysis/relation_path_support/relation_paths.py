...
import datasets
from dataset_analysis.relation_path_support import graph_paths
...
def compute(dataset, read_separator = ";"):
    """
        ...
        :param read_separator: the separator to use when reading the graph paths output file
    ...
    train_fact_to_one_step_graph_paths, train_fact_to_two_step_graph_paths = graph_paths.read_train(dataset)
    ...
    for fact_key in train_fact_to_one_step_graph_paths:
        ...
        # get its relation
        _, fact_relation, _ = fact_key.split(read_separator)
    ...
    for fact_key in train_fact_to_two_step_graph_paths:
        ...
        for two_step_graph_path in two_step_graph_paths:
            ...
            relation_paths_encountered_for_this_fact.add(step_one_rel + ";" + step_two_rel)
...
def save(dataset):
    ...
    for relation in dataset.relationships:
        ...
        if len(one_step_paths_and_counts) == 0:
            ...
            one_step_relation_path_lines.append(relation + "\t" + ",".join(one_step_paths_and_counts_strings) + "\n")
        ...
        if len(two_step_paths_and_counts) == 0:
            ...
            two_step_relation_path_lines.append(relation + "\t" + ",".join(two_step_paths_and_counts_strings) + "\n")
    ...
    with open(one_step_relation_paths_filepath, "r") as one_step_rel_paths_input_file:
        ...
        for line in lines:
            ...
            if not "\t" in line:
                ...
                relation, path_2_count_str = line.strip().split("\t")
                for path_and_count in path_2_count_str.split(","):
